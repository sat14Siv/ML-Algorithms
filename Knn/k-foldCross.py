# -*- coding: utf-8 -*-
"""
Created on Mon Jun 18 10:51:10 2018
...
def knn_prediction(train_data, test_data, k): # After finding out the optimal k for the data, this is used to classify the test data.
    predictions = []
    for sample_index in range(len(validation_data)):
        sample = validation_data.iloc[sample_index]
        predictions.append(get_predictions(sample, train_data, k))
...
if __name__ == '__main__':
    ...
    accur = []
    for train, validation in kfold.split(data):
        train_data = data.iloc[train]
        validation_data = data.iloc[validation][data.columns[:-1]]
        validation_class = data.iloc[validation][data.columns[-1]]
        train_data, validation_data = preProcess(train_data, validation_data)
        #k_req, scores = optimal_k(train_data, validation_data, validation_class, 4, 4)
     #Now, use the test data. In my case, there is no test data
        predictions = knn_prediction(train_data, validation_data, 9)
        accur.append(get_accuracy(predictions, validation_class))
        print(accur)
    accuracy = np.mean(accur)